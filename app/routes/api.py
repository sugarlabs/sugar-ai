...
from app.database import get_db, APIKey, TokenUsage
...
class PromptedLLMRequest(BaseModel):
    ...
    chat: bool = Field(
        False, description="Enable chat mode (uses messages instead of question)"
    )
    question: Optional[str] = Field(
        None, description="The question to ask (required if chat=False)"
    )
    custom_prompt: Optional[str] = Field(
        None,
        description="Custom prompt to replace system prompt (required if chat=False)",
    )
    messages: Optional[List[ChatMessage]] = Field(
        None, description="List of chat messages (required if chat=True)"
    )
...
def verify_api_key(
    api_key: Optional[str] = Header(None, alias="X-API-Key"), request: Request = None
):
    """Verify API key and check quota"""
    if not api_key:
        logger.warning(
            f"API key missing: {request.client.host if request else 'unknown'}"
        )
        raise HTTPException(status_code=401, detail="API key is missing")

    if api_key not in settings.API_KEYS:
        logger.warning(
            f"Invalid API key used: {api_key[:5]}... from {request.client.host if request else 'unknown'}"
        )
...
async def ask_question(
    question: str, user_info: dict = Depends(verify_api_key), request: Request = None
):
    """Process a question using RAG pipeline with token tracking"""
    ...
    logger.info(
        f"REQUEST - /ask - User: {user_info['name']} - IP: {client_ip} - Question: {question[:50]}..."
    )
    ...
    try:
        ...
        # Log completion
        logger.info(
            f"RESPONSE - User: {user_info['name']} - Success - "
            f"Time: {process_time:.2f}s - Tokens: {token_stats.get('total_tokens', 'N/A')}"
        )

        # Get remaining quota
        remaining = settings.MAX_DAILY_REQUESTS - user_quotas.get(api_key, {}).get(
            "count", 0
        )
...
async def ask_llm(
    ...
    client_ip = request.client.host if request else "unknown"
    logger.info(
        f"REQUEST - /ask-llm - User: {user_info['name']} - IP: {client_ip} - Question: {question[:50]}..."
    )
    ...
    try:
        if request_data.chat:
            ...
            if not request_data.messages:
                raise HTTPException(
                    status_code=400, detail="messages field is required when chat=True"
                )
            ...
            logger.info(
                f"REQUEST - /ask-llm-prompted (chat=True) - User: {user_info['name']} - IP: {client_ip} - Last message: {last_user_msg[:200]}..."
            )

            # Log system message if present
            system_messages = [
                msg for msg in request_data.messages if msg.role == "system"
            ]
            if system_messages:
                logger.info(
                    f"SYSTEM PROMPT - User: {user_info['name']} - Prompt: {system_messages[0].content[:100]}..."
                )
            ...
            process_time = time.time() - start_time
            logger.info(
                f"RESPONSE - User: {user_info['name']} - Success - Time: {process_time:.2f}s - Last message: {last_user_msg[:200]}..."
            )
            ...
            logger.info(
                f"REQUEST - /ask-llm-prompted - User: {user_info['name']} - IP: {client_ip} - Question: {request_data.question[:200]}..."
            )
            logger.info(
                f"CUSTOM PROMPT - User: {user_info['name']} - Prompt: {request_data.custom_prompt[:100]}..."
            )
...
async def debug(
    ...
    client_ip = request.client.host if request else "unknown"
    logger.info(
        f"REQUEST - /debug - User: {user_info['name']} - IP: {client_ip} - code: {code[:50]}..."
    )
...
async def change_model(
    ...
    client_ip = request.client.host if request else "unknown"
    logger.info(
        f"REQUEST - /change-model - API Key: {api_key[:5]}... - IP: {client_ip} - Model: {model}"
    )

    if api_key not in settings.API_KEYS:
        logger.warning(
            f"Invalid API key used for model change: {api_key[:5]}... from {client_ip}"
        )
        raise HTTPException(status_code=401, detail="Invalid API key")
    ...
    if not user_info.get("can_change_model", False):
        logger.warning(
            f"Unauthorized model change attempt by: {user_info['name']} from {client_ip}"
        )
        raise HTTPException(
            status_code=403, detail="User doesn't have permission to change model"
        )

    if password != settings.MODEL_CHANGE_PASSWORD:
        logger.warning(
            f"Invalid password for model change by: {user_info['name']} from {client_ip}"
        )
    ...
    try:
        ...
        logger.error(
            f"Error changing model to {model} by {user_info['name']}: {str(e)}"
        )
...
async def get_my_token_stats(
    ...
    if not agent or getattr(agent, "token_tracker", None) is None:
        raise HTTPException(status_code=500, detail="Token tracker not initialized")

    stats = agent.token_tracker.get_user_stats(api_key, days=days)

    return {"user": user_info["name"], "period_days": days, "statistics": stats}


@router.get("/usage/detailed-history")
async def get_detailed_usage_history(
    limit: int = Query(50, ge=1, le=500, description="Number of recent records"),
    user_info: dict = Depends(verify_api_key),
    request: Request = None,
):
    """Get detailed token usage history for current user"""
    api_key = request.headers.get("X-API-Key")
    ...
    try:
        ...
        records = (
            db.query(TokenUsage)
            .filter(TokenUsage.api_key == api_key)
...
@router.get("/usage/global-stats")
async def get_global_token_stats(
    days: int = Query(7, ge=1, le=365, description="Number of days to look back"),
    user_info: dict = Depends(verify_api_key),
):
    """
    Get global token usage statistics (all users)

    Requires: Admin privileges (can_change_model = True)
    """

    # Check if user has admin privileges
    if not user_info.get("can_change_model", False):
        raise HTTPException(
            status_code=403, detail="Admin privileges required to view global stats"
        )

    if not agent or getattr(agent, "token_tracker", None) is None:
        raise HTTPException(status_code=500, detail="Token tracker not initialized")

    stats = agent.token_tracker.get_global_stats(days=days)
