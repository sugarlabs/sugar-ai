...
| `/chat/completions` | Accepts chat history with system prompt | JSON body | • Send chat history along with system prompt<br>• Configurable model parameters |
...
- **Chat Completion POST endpoint (/chat/completions)**

    If you want to send a conversation history and generate a model response then use this endpoint. The structure of the request and response is kept similar to any popular API providers. 
    - Send chat history in a list tagged with `user` and `assistant` roles
    - System prompt will be sent at the top of the history with the role being tagged as `system`
